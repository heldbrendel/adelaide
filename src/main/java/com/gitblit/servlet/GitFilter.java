...
public class GitFilter extends AccessRestrictionFilter {
    ...
    protected static final String[] suffixes = {
            gitReceivePack, gitUploadPack, gitLfs,
            "/info/refs", "/HEAD", "/objects"};
    ...
    public static String getRepositoryName(String value) {
        log.debug("--> getRepositoryName(value = [{}])", value);
        ...
        if (repository.contains("/")) {
            repository = repository.substring(repository.indexOf("/"));
        }

        log.debug("<-- getRepositoryName: {}", repository);
        return repository;
    }
    ...
    protected String extractRepositoryName(String url) {
        log.debug("--> extractRepositoryName(url = [{}])", url);
    ...
    protected String getUrlRequestAction(String suffix) {
        log.debug("--> getUrlRequestAction(suffix = [{}])", suffix);
    ...
    protected UserModel getUser(HttpServletRequest httpRequest) {
        log.debug("--> getUser(httpRequest = [{}])", httpRequest);
    ...
    protected boolean isCreationAllowed(String action) {
        log.debug("--> isCreationAllowed(action = [{}])", action);

        //Repository must already exist before large files can be deposited
        if (action != null && action.equals(gitLfs)) {
            return false;
        }
    ...
    protected boolean isActionAllowed(RepositoryModel repository, String action, String method) {
        log.debug("--> isActionAllowed(repository = [{}], action = [{}], method = [{}])", repository, action, method);

        // the log here has been moved into ReceiveHook to provide clients with
        // error messages
        if (gitLfs.equals(action)) {
            return method.matches("GET|POST|PUT|HEAD");
        }
    ...
    protected boolean requiresClientCertificate() {
        log.debug("--> requiresClientCertificate()");
    ...
    protected boolean requiresAuthentication(RepositoryModel repository, String action, String method) {
        log.debug("--> requiresAuthentication(repository = [{}], action = [{}], method = [{}])", repository, action, method);
    ...
    protected boolean canAccess(RepositoryModel repository, UserModel user, String action) {
        log.debug("--> canAccess(repository = [{}], user = [{}], action = [{}])", repository, user, action);
    ...
    protected RepositoryModel createRepository(UserModel user, String repository, String action) {
        log.debug("--> createRepository(user = [{}], repository = [{}], action = [{}])", user, repository, action);
    ...
    protected String getAuthenticationHeader(HttpServletRequest httpRequest, String action) {
        log.debug("--> getAuthenticationHeader(httpRequest = [{}], action = [{}])", httpRequest, action);
    ...
    protected boolean hasValidRequestHeader(String action, HttpServletRequest request) {
        log.debug("--> hasValidRequestHeader(action = [{}], request = [{}])", action, request);
