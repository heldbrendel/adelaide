...
public class EditTeamPage extends RootSubPage {
    ...
    protected void setupPage(final TeamModel teamModel) {
        ...
        // users palette
        final Palette<UserChoice> users = new Palette<>("users",
                new ListModel<>(getTeamUsers(teamUsers)),
                new CollectionModel<>(sortByDisplayName(getTeamUsers(app().users().getAllUsernames()))),
                new ChoiceRenderer<>(null, "userId"), 10, false);
        users.add(new DefaultTheme());
        ...
        final Palette<String> postReceivePalette = new Palette<>("postReceiveScripts",
                ...
                new CollectionModel<>(app().repositories().getPostReceiveScriptsUnused(null)),
                new StringChoiceRenderer(), 12, true);
