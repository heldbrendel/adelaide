...
public class EditRepositoryPage extends RootSubPage {
    ...
    protected void setupPage(RepositoryModel model) {
        ...
        final Palette<UserChoice> ownersPalette = new Palette<>("owners",
                new ListModel<>(owners),
                new CollectionModel<>(persons),
                new ChoiceRenderer<>(null, "userId"), 12, false);
        ownersPalette.add(new DefaultTheme());
        ...
        final Palette<String> indexedBranchesPalette = new Palette<>("indexedBranches",
                new ListModel<>(indexedBranches),
                new CollectionModel<String>(allLocalBranches),
                new StringChoiceRenderer(), 8, false);
        indexedBranchesPalette.add(new DefaultTheme());
        indexedBranchesPalette.setEnabled(luceneEnabled);

        // federation sets palette
        List<String> sets = app().settings().getStrings(Keys.federation.sets);
        final Palette<String> federationSetsPalette = new Palette<>("federationSets",
                new ListModel<>(federationSets),
                new CollectionModel<>(sets),
                new StringChoiceRenderer(), 8, false);
        federationSetsPalette.add(new DefaultTheme());

        // pre-receive palette
        if (!ArrayUtils.isEmpty(repositoryModel.preReceiveScripts)) {
            preReceiveScripts.addAll(repositoryModel.preReceiveScripts);
        }
        final Palette<String> preReceivePalette = new Palette<>("preReceiveScripts",
                new ListModel<>(preReceiveScripts),
                new CollectionModel<>(app().repositories().getPreReceiveScriptsUnused(repositoryModel)),
                new StringChoiceRenderer(), 12, true);
        preReceivePalette.add(new DefaultTheme());

        // post-receive palette
        if (!ArrayUtils.isEmpty(repositoryModel.postReceiveScripts)) {
            postReceiveScripts.addAll(repositoryModel.postReceiveScripts);
        }
        final Palette<String> postReceivePalette = new Palette<>("postReceiveScripts",
                new ListModel<>(postReceiveScripts),
                new CollectionModel<>(app().repositories().getPostReceiveScriptsUnused(repositoryModel)),
                new StringChoiceRenderer(), 12, true);
        postReceivePalette.add(new DefaultTheme());
