...
public abstract class RootPage extends BasePage {
    ...
    private PageParameters getRootPageParameters() {
        if (reusePageParameters()) {
            ...
            if (pp != null) {
                ...
                if (params.getNamedKeys().contains("db")
    ...
    private void loginUser(UserModel user) {
        if (user != null) {
            ...
            HttpServletResponse response = ((ServletWebResponse) RequestCycle.get().getResponse()).getContainerResponse();
            ...
            request = ((ServletWebRequest) RequestCycle.get().getRequest()).getContainerRequest();
            response = ((ServletWebResponse) RequestCycle.get().getResponse()).getContainerResponse();
    ...
    protected List<MenuItem> getTimeFilterItems(PageParameters params) {
        ...
        if (!clonedParams.getNamedKeys().contains("db")) {
            clonedParams.get("db");
        }
    ...
    protected List<RepositoryModel> getRepositories(PageParameters params) {
        ...
        int daysBack = params.get("db").toInt();
        int maxDaysBack = app().settings().getInteger(Keys.web.activityDurationMaximum, 30);
